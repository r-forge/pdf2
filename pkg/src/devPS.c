 ...
 *  Copyright (C) 2008--2011  Tadashi Kadowaki
 *
...
static SEXP getFontDB(const char *fontdbname) {
    ...
    SEXP fontdb;
    PROTECT(graphicsNS = R_FindNamespace(ScalarString(mkChar("pdf2"))));
			 ...
			 PostScriptDesc *pd)
{
	...
	SEXP graphicsNS = R_FindNamespace(ScalarString(mkChar("pdf2")));
...
static void PDF_endpage(PDFDesc *pd)
{
    ...
    fprintf(pd->pdffp, "%d 0 obj\n%d\nendobj\n", pd->nobjs,
	    here - pd->startstream);
    /*
     * Following (1)+3 lines are moved from PDF_NewPage
     * to make insertion of annotation object easily
     */
    PDF_pos_grow(pd);
    pd->pos[++pd->nobjs] = (int) ftell(pd->pdffp);
    fprintf(pd->pdffp, "%d 0 obj\n<<\n/Type /Page\n/Parent 3 0 R\n/Contents %d 0 R\n/Resources 4 0 R\n",
...
static void PDFSimpleText(double x, double y, const char *str,
    ...
    int face = gc->fontface;
    double a, b, rot1;
    /* double ascent, descent, width; # kadowaki */
    ...
    pd->text_x = x;
    pd->text_y = y;
    PDF_StrSize(str, gc, dd, &(pd->text_ascent), &(pd->text_descent), &(pd->text_width));
    /* PDF_StrSize(str, gc, dd, &ascent, &descent, &width);
    pd->text_ascent = ascent;
    pd->text_descent = descent;
    pd->text_width = width; kadowaki */
}
...
/*
 * Should be called bofore "pd->pos[++pd->nobjs]" to prevent buffer over run
 * PDF annotation make many objects so that number of objects easily exceeds
 * (6*pd->pagemax + 550) that is the number of objects prepared when new page
 * is created by PDF_NewPage
 */
static void PDF_pos_grow(PDFDesc *pd)
{
    int *pos;
